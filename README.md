...
| Driving the process | `RuntimeService` / `TaskService` | an **autogenerated** REST client against `/engine-rest` |
| Process contract | generated inside the app | generated inside the **worker**, which owns the whole model |
...
timer**, a **parallel fork/join**, a **user task with a Camunda Form**, an **execution listener** and a
**task listener** (which — unlike the service tasks — run inside the engine), **compensation / SAGA**
...
- **Driving the process is done over REST — through an autogenerated client.**
  ...
  `/engine-rest`, all correlated by the **`ApplicationId` business key**, exactly as the embedded adapter
  does through the Java API. The REST calls go through a **typed client generated from CIB seven's
...
- **Model validation** (`bpmn-to-code-testing`) checks the `.bpmn` models structurally at build time in
  ...
  **external task with a topic** (the remote counterpart to the embedded delegate-expression rule).
